...
{
    int  v; // item is the key in the graph
    int d; // priority is the distance
} MinHeapNode;
...
void SiftUp(MinHeap* h, int v, int d) //v -> node key; d -> new distance in graph
{
    int i = h->pos[v];
    h->elem[i]->d = d;
...
void minHeapInsert(MinHeap* h, int v, int d) //v -> node key; d -> distance
{
