...
outNeighbors node graph = S.filter (\x -> S.member (node, x) (edges graph)) (nodes graph)
...
inNeighbors node graph = S.filter (\x -> S.member (x, node) (edges graph)) (nodes graph)
...
removeNode node graph = (S.delete node (nodes graph), S.filter (\x -> (&&) (node /= fst x) (node /= snd x)) (edges graph))
...
splitNode old news graph
    | null news = removeNode old graph
    | otherwise = (S.union (S.delete old (nodes graph)) (S.fromList news), S.fromList (concatMap (\a -> map (\b -> 
        (if old == fst b then (a, snd b) 
        else (if old == snd b then (fst b, a)
        else b))
    ) (S.toList (edges graph))) news))
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = if S.null (S.filter prop (nodes graph)) then graph 
    else (S.insert node (S.filter (not . prop) (nodes graph)), S.fromList (map (\e ->
    (if prop (fst e) && prop (snd e) then (node, node)
        else (if prop (fst e) then (node, snd e)
            else (if prop (snd e) then (fst e, node) 
            else e)))) (S.toList (edges graph))))