...
-- Q1: Cum ați defini o clasă care să surprindă conceptul de graf orientat? 
-- Ați parametriza-o cu tipul concret (de exemplu, AlgebraicGraph a) sau cu
-- constructorul de tip în sine (de exemplu, AlgebraicGraph)?

-- R1: As parametriza-o cu tipul concret deoarece permite customizarea parametrului
-- a, pe cand definirea printr-un constructor ar avea tipul de date prestabilit.

-- Q2: Cum ar arăta tipul funcției nodes dacă ar fi definită în interiorul 
-- acestei clase?
...
splitNode node targets 
    | length targets > 1 || null targets = extend (\n -> if n == node then spltNode targets Empty else Node n)  
    | otherwise = extend (\n -> if n == node then Node (head targets) else Node n)
    where 
        spltNode [] node = node
