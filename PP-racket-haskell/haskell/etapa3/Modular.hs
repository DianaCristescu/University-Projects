...
isModule' :: Ord a
         ...
         -> Graph a
         -> Bool
isModule' set set' ins outs graph
    | S.null set' = True
    | S.member first set = (ins == ins' && outs == outs') && isModule' set rest ins outs graph
    | otherwise = isModule' set rest ins outs graph
    where
        list = S.toList set'
        first = head list
        rest = S.fromList (tail list)
        ins' = S.filter (\n -> not $ S.member n set) (inNeighbors first graph)
        outs' = S.filter (\n -> not $ S.member n set) (outNeighbors first graph)

isModule :: Ord a
         => S.Set a
         -> Graph a
         -> Bool
isModule set graph = isModule' set rest ins outs graph
    where
        list = S.toList set
        first = head list
        rest = S.fromList (tail list)
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = S.foldl (&&) True (S.map (`isModule` graph) partition)
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = minimumBy (compare `on` S.size) (filter (\s -> S.size s /= 1) $ S.toList $ S.fromList $ filter (`isModularPartition` graph) partitions)
