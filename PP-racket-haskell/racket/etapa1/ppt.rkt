...
(define (apply-matrix-transformations Ts ppt)
  (if (null? Ts)
      ...
      (let ((T (car Ts))
            (Ts (cdr Ts)))
        (cond
          [(= T 1) (apply-matrix-transformations Ts (multiply T1 ppt))]
          [(= T 2) (apply-matrix-transformations Ts (multiply T2 ppt))]
          [else (apply-matrix-transformations Ts (multiply T3 ppt))])
        )))
