...
(define (get-transformations-helper n acc)
  (if (= 1 n)
      acc
      (let ((n (quotient (+ n 1) 3))
            (Ts (+ (remainder (+ n 1) 3) 1)))
        (get-transformations-helper n (cons Ts acc)))
      ))
...
(define get-nth-tuple
  (λ (tuple X1 X2 X3)
    (λ (n)
      (let* ((transformations (get-transformations n))
             (Fs (map (λ (x) (cond [(= x 1) X1]
                                     [(= x 2) X2]
                                     [else X3]))
                        transformations)))
...
(define get-nth-ppt-from-matrix-transformations
  (λ (n)
    (let ((FT1 (λ (t) (multiply T1 t)))
          (FT2 (λ (t) (multiply T2 t)))
          (FT3 (λ (t) (multiply T3 t))))
...
(define get-nth-quadruple
  (λ (n)
    (let ((FQ1 (λ (t) (apply Q1 t)))
          (FQ2 (λ (t) (apply Q2 t)))
          (FQ3 (λ (t) (apply Q3 t))))
      ((get-nth-tuple '(1 1 2 3) FQ1 FQ2 FQ3) n))))


; TODO
; Folosiți rezultatul întors de get-nth-quadruple pentru a 
; obține al n-lea TPP din arbore. [a = gh,   b = 2ef,   c = e^2 + f^2 ; (g, e, f, h)-quad ; (a, b, c)-TPP]
