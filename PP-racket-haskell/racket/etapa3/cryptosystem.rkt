...
(define (apply-fct Fs e f)
  (map (λ (x) (x e f)) Fs))

(define (key n)
  (let* ((quad (get-nth-quadruple n))
         (e (car (cdr quad)))
         (f (car (cdr (cdr quad)))))
    (map (λ (x) (modulo x 27)) (apply-fct (list a1 b1 c1 a2 b2 c2 a3 b3 c3) e f))))
...
(define (message->codes message)
  (map (λ (x) (if (= x 32) 0 (- x 96))) (map char->integer (string->list message))))
...
(define (codes->message codes)
  (list->string (map integer->char (map (λ (x) (if (= x 0) 32 (+ x 96))) codes))))  
...
(define (extend-key key size)
  (let iter ((i size) (new_key '()) (k key))
    (if (= i 0)
        new_key
        (if (null? k)
            (iter (- i 1) (append new_key (list (car key))) (cdr key))
            (iter (- i 1) (append new_key (list (car k))) (cdr k))))))
...
(define (apply-key codes key opperation)
  (map opperation (extend-key key (length codes)) codes))

(define encrypt-codes
  (λ (message key)
    (apply-key message key (λ (m k) (modulo (+ k m) 27)))))
(define decrypt-codes
  (λ (crypted key)
    (apply-key crypted key (λ (c k) (modulo (- k c) 27)))))
